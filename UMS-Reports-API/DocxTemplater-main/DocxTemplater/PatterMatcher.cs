    ...
    internal static class PatternMatcher
    {
        ...
        private static readonly Regex PatternRegex = new(@"\{\s*(?<condMarker>\?\s*)?\{\s* # a leading ? indicates a condition
                                                                (?:   
                                                                    (?<separator>:\s*s\s*:) |
                                                                    (?<else>(?:else)|:) |
                                                                    (?<vmerge>vmerge)|  # New match for {{vmerge}}
                                                                    (?<vmerge>hmerge)|  # New match for {{hmerge}}
        ...
        public static IEnumerable<PatternMatch> FindSyntaxPatterns(string text)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                else if (match.Groups["vmerge"].Success) // Detect {{vmerge}}
                {
                    result.Add(new PatternMatch(match, PatternType.VMerge, null, null, "vmerge", null, null, match.Index, match.Length));
                }
                else if (match.Groups["hmerge"].Success) // Detect {{hmerge}}
                {
                    result.Add(new PatternMatch(match, PatternType.VMerge, null, null, "hmerge", null, null, match.Index, match.Length));
                }
                else
                {
